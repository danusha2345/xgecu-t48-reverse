...
- Дата сборки + слово версии — ровно то, что Xgpro читает, чтобы решить,
...
- **Новое:** top-opcode **`0x00` = device identify** (дата сборки +
  серийник + версия); стабильная форма заголовка EP1-ответа `… 01 07 00`
  с кодом модели в байте[6].
