...
Устройство представилось так: сборка прошивки **`2024-08-15 17:21`**,
версия **`0x05D1` (1489)**, модель T48. Энумерация прошла на **high-speed
USB 2.0 (480 Мбит/с)** ровно с **четырьмя bulk-эндпоинтами** —
`0x01`/`0x81` и `0x02`/`0x82`, все с `wMaxPacketSize = 512` — что
**напрямую подтверждает карту эндпоинтов из §2** (EP1 — короткие команды,
EP2 — bulk).
...
> обновление — захватывай **весь root-hub**, а не устройство. В этой
> сессии устройство лишь сообщило свою (уже актуальную) версию, так что
> ничего не упущено.
...
- Байт заголовка `[4]` **непостоянен**: `0x03` в захвате сразу после
  холодного подключения, `0x27` при каждом чтении после какой-либо
  активности — похоже на зависимость от состояния/режима, смысл TBD.
- **Идентификатор сборки прошивки — это строка-таймстамп** (`2024-08-15
  17:21` здесь), и она **не менялась** ни в одном захвате.
- Хвостовое 16-битное слово — это **НЕ версия прошивки**, а **живой
  ADC-подобный замер**. При трёх чтениях подряд вернулось `0x05E7 /
  0x05ED / 0x05E9` (1511 / 1517 / 1513), а в раннем захвате `0x05D1`
  (1489): дрожь в несколько отсчётов вокруг ~1510, т.е. аналоговое
  значение, снимаемое в момент identify, а не номер ревизии. **По `0x00`
  нельзя определить, прошилось ли устройство** — единственный стабильный
  идентификатор (дата сборки) не меняется при перепрошивке на ту же
  сборку, так что подтвердить обновление можно только захватом всей шины
  (врезка в начале §31).
