...
| READID | `05 00 00 00 …` | `07 …80 80 ff c0` **CID** `44 00 01 45 30 30 34 47 39 …` | **CID** — но это 32-бит LE-слова: реальный декод = SanDisk `DG40…` (см. §33.7), а не `"E004G9"` |
...
### 33.7 CID/CSD приходят четырьмя 32-битными LE-словами (а не плоским регистром)

16 байт CID/CSD после 8-байт заголовка ответа (`READID 0x05`, `READCSD 0x06`) —
это **не** плоский JEDEC-регистр MSB-first. T48 отдаёт их **четырьмя 32-битными
little-endian словами**: каждая 4-байтовая группа развёрнута относительно JEDEC.
Разбор сырых 16 байт как есть даёт **мусорный MID и нечитаемый PNM** (и неверную
дату изготовления).

Доказательство — CRC7 (`x^7+x^3+1`) по первым 15 байтам сходится с хранимым CRC
**только** после разворота каждого 4-байт слова, причём сразу у **обоих**
регистров независимо (не совпадение):

| захват | сырые 16 Б (как пришли) | прямой разбор | **разбор с word-swap** | CRC7 |
|---|---|---|---|---|
| DJI Air 3S | `5900019b 363053305e 7c0034fc bb6945` | MID `0x59` (Unknown), PNM `·60S0^`, **2019-09** | **MID `0x9B` YMTC, PNM `Y0S064`, 2024-11**, SN `0x7C5E4569` | swap ✓ / прямой ✗ |
| §33.2 `…E004G9` | `44000145 30303447 39…` | MID `0x44` (Unknown), PNM `E004G9` | **MID `0x45` SanDisk, PNM `DG40…`** | — |

То есть пример из §33.2 сам прочитан в неверном порядке: тот чип —
**SanDisk** (`MID 0x45`, `DG40…`), а не неопознанный `0x44`/`"E004G9"`. Фикс —
разворот 4-байт слов перед разбором (`_decode_cid()` в
`examples/t48_emmc_isp.py`). **EXT_CSD** (512-байт блок на EP2 IN) — это байтовый
поток, а не регистр, его разворачивать **не** надо (`SEC_COUNT@212` читается
верно как есть).
...
## 36. Большие чтения — стриминг, переарм и предел дренажа хоста

Отреверсено по **нативному чтению 1 ГБ** на 40 МГц / 1-бит (`чтение_1гб…pcapng`),
проверено на железе против Rust-порта.

**Большое чтение = один `RD_SETUP` с огромным `n_chunks`, стримом.** Чтение 1 ГБ
использовало **5 `RD_SETUP`-ов** с `n_chunks` (byte[16..20]) = `256, 256, 65536,
256, 256`; именно `65536`-й несёт весь 1 ГБ. Его payload — это просто `read 16384
(16-байт заголовок + 16368 данных) + read 16 (хвост)`, **повторённый без переарма**
~10 000+ чанков подряд. То есть большому чтению переарм **не нужен** — оно стримит.

**Переарм существует, но только для маленьких `256`-чанковых `RD_SETUP`-ов**
(другая операция — возможно, verify/scan-проход). Там цикл, на каждое
`32768`-чтение (2 чанка): `read 32768 + read 16, затем RD_14 → ep2_send(512 нулей)
→ RD_15`, где у цикл-`RD_15` **byte[8] = 0x40** (у начально-arm-`RD_15`
byte[8] = 0x01). Вставка этого переарма в *стримовое* чтение ломает его (устройство
посреди стрима не ждёт OUT) — так что не надо.

**Реальный потолок для порта — скорость дренажа USB на хосте.** eMMC выдаёт данные
на скорости шины (~5 МБ/с на 40 МГц 1-бит). Нативная (WinUSB) дренажит достаточно
быстро, чтобы один `65536`-чанковый `RD_SETUP` тёк на весь 1 ГБ. **libusb**-порт
(Linux или rusb на Windows) дренажит чуть медленнее (~4.5 МБ/с, два bulk-IN на
чанк), поэтому на длинном `RD_SETUP` внутренний read-FIFO программатора
**переполняется и заклинивает** (ошибка `ep1_send`/`ep2_send` → физический replug).
Измеренный предел на этой установке: **256 чанков (4 МБ) на `RD_SETUP` безопасно;
512 нестабильно; 1024 клинит.** Поэтому libusb-порт должен ограничивать каждый
`read_sectors`/`RD_SETUP` ~256 чанками и **крутить много коротких `RD_SETUP`-ов
внутри одной переиспользуемой сессии** для больших объёмов — проверено надёжно на
256 МБ при 4.55 МБ/с (≈ нативная). (Нативная обходит лимит чисто за счёт более
быстрого дренажа, без всяких лишних команд.)
